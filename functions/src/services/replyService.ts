...
const replyCollection = 'communityPostReplies';

export const createReply = async (reply: Omit<Reply, 'id'>): Promise<Reply> => {
    const replyRef = admin.firestore().collection(replyCollection).doc();
...
export const updateReply = async (
    ...
    const replyRef = admin.firestore().collection(replyCollection).doc(replyId);
...
export const getReplies = async (
    ...
    if (lastVisible) {
        const lastVisibleDoc = await admin.firestore().collection(replyCollection).doc(lastVisible).get();
